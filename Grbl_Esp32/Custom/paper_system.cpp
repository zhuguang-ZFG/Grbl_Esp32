...
Error user_m_code(uint16_t code) {
    switch (code) {
        case 701: {
            bool paper_present   = digitalRead(PAPER_SENSOR_PIN);   // HIGH = 有纸, LOW = 无纸
        ...
        case 711: {
            ...
            grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "M711: 拾落电机点动完成");
            return Error::Ok;
        }
        case 712: {
            // 面板电机点动
            ...
            grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "M712: 面板电机点动完成");
            return Error::Ok;
        }
        case 713: {
            // 进纸器电机点动
            ...
            grbl_msg_sendf(CLIENT_SERIAL, MsgLevel::Info, "M713: 进纸器电机点动完成");
            return Error::Ok;
        }
        default:
...
void user_defined_macro(uint8_t index) {
    if (index == 0) {
        bool paper_present = digitalRead(PAPER_SENSOR_PIN);
        grbl_msg_sendf(CLIENT_SERIAL,
                       MsgLevel::Info,
                       "纸张传感器(Macro0): %s",
                       paper_present ? "有纸" : "无纸");
