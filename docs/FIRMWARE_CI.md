...
- 整片量产（本地构建产物，非 CI artifact）：每次 `pio run`（release）都会由 `merge_firmware.py`（`buildprog` post-action）生成  
  ...
  **坑与修复：** `merge_firmware.py` 必须 **UTF-8**。若写成 UTF-16，合包不跑、旧 `firmware_full_0x0.bin` 时间戳不更新。  
  检查：`python -c "print(open('merge_firmware.py','rb').read(8))"` → 应为 `b'# merge_'`。  
  修复：`git checkout HEAD -- merge_firmware.py` 后重新 `platformio run -e release`。完整 SOP 见 [`docs/PROJECT_LESSONS.md`](PROJECT_LESSONS.md)「0x0 合包」。
